defmodule BackendWeb.ServeData do
  ...
  def getOrganizations(conn, _params) do
    json conn, BackendWeb.DatabaseManager.runCypher("match (o: Organization) return ID(o) as ordID, o.title as title, o.slots_per_bucket as slots_per_bucket")
  end

  def getObjectives(conn, %{"orgTitle" => organizationTitle} = _params) do
    json conn, BackendWeb.DatabaseManager.runCypher(
  ...
  def getInPathTasks(conn, %{"tasks-for" => degrees, "at" => organizationTitle} = _params) do
    json conn, Enum.into(
      Enum.map(
        BackendWeb.DatabaseManager.runCypher(
