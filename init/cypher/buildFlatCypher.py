...
class Inserter():
    ...
    def insertOrg(self):
        def createOrgInsert():
            ...
            s += "CREATE (ob :Objective :Executable {title: \"B.S in Computer Science\"})\n"
            s += "CREATE (wsu)-[:OWNS]->(ob)\n"
            return self.cleanInsert(s) 
        ...
        with open (file, "r") as f:
            for line in f:
                ...
                for index, p in enumerate(paths):
                    for child in p:
                        path += f"CREATE ({parent.task_id})-[:REQUIRES " + "{" + f"pathID: {index}" + "}" + f"]->({child.task_id})\n"
    ...
    def buildObjective(self, exe: dict[str, Task]):
        ...
        for p in list(map(lambda s: exe[s], paths.split(' '))):
            objective += "CREATE (ob)-[:REQUIRES" + "{" + f"pathID: {0}" + "}" + f"]->({p.task_id})\n"
